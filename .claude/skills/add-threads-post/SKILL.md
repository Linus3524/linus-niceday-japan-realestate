...
依貼文主旨放進既有分類，順序接在該分類末尾。現有分類（標籤列由左至右即此順序，
依「先看行情選地 → 懂規則 → 申請入住 → 買房」的閱讀動線排列）：
Linus 知識系列、申請流程圖解、東京租金行情、寶藏車站、租屋潛規則、
台日大不同、審查與申請、入住與退租、打工度假、日本買房、房仲日常。

各分類的收錄界線（名稱相近，歸類前先確認）：

| 分類 | 收什麼 | 不收什麼 |
| --- | --- | --- |
| Linus 知識系列 | 認真製圖、成系列的長篇主題文，含生活觀察隨筆；租買兩邊通吃 | 隨手記的短篇心得 |
| 申請流程圖解 | 「日本賃貸完全解析」那組申請流程圖文 | 其他單篇知識文 |
| 東京租金行情 | 租金水準、漲跌、初期費用預算、格局供給變化 | 單一車站的生活機能介紹 |
| 寶藏車站 | 以「某個車站」為主角的居住地推薦 | 全區域的行情統計 |
| 租屋潛規則 | 日本租賃市場的運作規則：先行契約、番手、外國籍條件、圖紙話術 | 純粹的台日觀念比較 |
| 台日大不同 | 台灣人帶著台灣經驗來會誤解的事：築年、風水、治安、附燈、二人入居 | 市場機制本身的說明 |
| 審查與申請 | 海外／境內審查、保證會社、申請文件與流程 | 簽約後的入住手續 |
| 入住與退租 | 簽約後的實務：入住當天、印章、水電 NHK、退房原狀回復 | 審查階段的注意事項 |
| 房仲日常 | Linus 的第一線見聞、客人互動、伴手禮、業界觀察 | 可整理成通則的知識 |

注意 `isContextMatch()` 的規則：`日本買房` 只出現在買房情境；
`Linus 知識系列` 兩邊都會出現；其餘皆視為租屋。**分類直接決定曝光面**。

這兩個分類名在 `featuredThreads.ts` 匯出為 `BUY_CATEGORY_LABEL` 與 `CROSS_CATEGORY_LABEL`，
`threadSearch.ts` 與測試都引用常數。**日後要改這兩類的名稱，只改常數值即可，不要再寫死字串。**
