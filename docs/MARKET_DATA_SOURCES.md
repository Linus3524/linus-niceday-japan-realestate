...
- 買房概算：以國土交通省「不動產資訊資料庫」XIT001 交易價格為第一順位。API 尚未核准或樣本不足時，畫面會明示仍使用租金收益率模型。
...
| 國交省不動產資訊資料庫 API | 實際交易價格、成約價格 | 季度資料；實際更新日看官方公告 | 每季更新 | API 核准並取得金鑰後啟用 |
...
API 核准後，把金鑰存入伺服器端 `.env.local`：

```bash
MLIT_REINFOLIB_API_KEY=取得的金鑰 npm run data:update:mlit-buy
npm run test:market-sources
npm run lint
```

更新程式會讀取 XIT001 的「不動產交易價格」資料，篩選公寓交易，按本站行政區及 1R／1K／1LDK／2LDK／3LDK+ 集計中位數；同一組至少 5 筆才寫入快照。間取り缺值時，才以文件化的面積帶分類。API 金鑰不會進入前端 bundle。
