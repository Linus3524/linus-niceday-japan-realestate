...
你代表他在網站上回答問題；需要真人一對一服務時，請引導使用者加 LINE：linus0922 與 Linus 本人聯繫。
...
3. 【重要防呆規則】對於「具體事實類」問題——例如審查所需文件的細節、費用金額、簽證與貸款條件、法規天數、印鑑或證明文件的規格等——你「只能」引用下方數據中明確寫出的內容作答，並一併說明資料日期、適用條件，以及需要向哪個單位（管理公司、保證公司、銀行、稅務署或自治體）確認。如果數據沒有寫，請「絕對不要自己憑空編造或推測具體細節」（例如不可自行杜撰「需要英文版文件」「需要公證」「需要某某表格」這類數據未提及的規格），而應誠實說明：「這部分的細節建議直接加 Linus 的 Line (linus0922) 為您確認最新狀況，以免資訊有誤喔」。
...
5. 若涉及具體案件諮詢，或用戶需要更進一步客製化置產或配對房源，請熱心主動邀請對方添加 LINE（帳號：linus0922）。提到聯絡 Linus 時必須明確寫出「LINE：linus0922」，網站會自動在回答下方顯示「一鍵加好友」按鈕。但每則回答最多只能出現一次聯絡邀請，也絕不能用聯絡邀請取代應該給的實質回答。
...
// 這是「送進 AI 前」的主題白名單，沒命中任何一組就直接被擋在本地，不會呼叫 Gemini。
// 它只是保護額度用的粗篩，真正的角色與內容限制在 systemInstruction（規則 6 角色鎖）。
//
// 【維護重點】白名單漏字的症狀是「使用者問了合理問題卻收到罐頭拒絕」，而且很難察覺，
// 因為看起來像 AI 不想回答。實測就抓到過三次：問「你是誰」、問「世田谷區住起來如何」、
// 問「池袋到新宿怎麼搭車」全部被誤擋，而這三種 persona 都有明文規定該怎麼回答。
// 所以：
//   1. 新增或修改前端「熱門諮詢」按鈕時，務必確認新句子至少命中一組。
//   2. 在 systemInstruction 新增「允許回答某類問題」的規則時，回來這裡補對應關鍵字。
//   3. 日文漢字與中文寫法都要列（審査／審查、税／稅），才不會漏掉。
const TOPIC_PATTERNS: RegExp[] = [
  // 租賃與買賣本體
  /租|買房|買屋|賣房|售屋|不動產|房地產|物件|房源|公寓|住宅|套房|房東|仲介|管理公司|保證公司|保證會社|契約|房租|家賃|押金|敷金|禮金|管理費|共益費|初期費用|格局|屋齡|坪|平方米|平方公尺|reins|suumo|マンション|アパート/i,
  // 審查、簽證與身分
  /審查|審査|簽證|簽証|在留|居留|打工度假|留學|留学|存款|財力|入住|退房|解約|搬家/,
  // 金流：貸款、銀行、稅務（稅金原本整個漏掉，固定資產稅這種核心買房問題會被擋）
  /房貸|貸款|銀行|利率|頭期|自備款|稅|税|課稅|報稅|節稅|固定資產|都市計畫/,
  // 營運與生活設施
  /民泊|旅館|airbnb|民宿|水電|瓦斯|網路|建築|土地|投資/i,
  // 區域與生活圈（規則 4：區域氛圍屬於可回答的軟性問題）
