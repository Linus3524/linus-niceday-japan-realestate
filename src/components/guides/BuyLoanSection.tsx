                        ...
                        <div key={bIdx} className="border border-[#DDE3DF] hover:border-[#00a174] bg-white transition-all duration-300 hover:shadow-colored-soft overflow-hidden">
                          ...
                          >
                            ...
                            <span className="flex items-center gap-3">
                              ...
                              <span className="text-xs font-bold font-sans text-[#66736C]">{expandedBanks.has(`overseas-${bIdx}`) ? "收合" : "查看條件"}</span>
                              <ChevronDown className={`h-4 w-4 text-[#66736C] transition-transform ${expandedBanks.has(`overseas-${bIdx}`) ? "rotate-180" : ""}`} />
                            </span>
                          </button>

                          {expandedBanks.has(`overseas-${bIdx}`) && (<>
                          <div className="grid grid-cols-1 sm:grid-cols-3 border-b border-[#DDE3DF] font-sans">
                            <div className="p-4 bg-[#F5F8F6] border-b sm:border-b-0 sm:border-r border-[#DDE3DF]">
                              <p className="text-[10px] font-bold tracking-wide text-[#66736C]">申貸對象</p>
                            ...
                            <div className="p-4 border-b sm:border-b-0 sm:border-r border-[#DDE3DF]">
                              <p className="text-[10px] font-bold tracking-wide text-[#66736C]">起貸金額／最高成數</p>
                              <p className="mt-1 text-xs font-semibold leading-relaxed text-[#00a174]">{bank.amountLimit}</p>
                            </div>
                            <div className="p-4 bg-[#F5F8F6]">
                              <p className="text-[10px] font-bold tracking-wide text-[#66736C]">最長貸款期限</p>
                          ...
                          <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-5 font-sans text-xs">
                            <div className="border border-[#DDE3DF]">
                              ...
                              <dl className="divide-y divide-[#ECEFEC]">
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">年齡</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.ageLimit}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">收入／資產</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.incomeAsset}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">對保／開戶</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.signingReq}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">還款／租金帳戶</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.rentAccount}</dd></div>
                              </dl>
                            </div>
                            <div className="border border-[#DDE3DF]">
                              <p className="bg-[#F5F8F6] px-3 py-2 font-bold text-[#1A2A22]">物件與貸款條件</p>
                              <dl className="divide-y divide-[#ECEFEC]">
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">物件／屋齡</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.propertyReq}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">承作區域</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.areaLimit}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">還款方式</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.repayment}</dd></div>
                                <div className="px-3 py-2.5"><dt className="font-bold text-[#00a174]">提前清償費</dt><dd className="mt-1 leading-relaxed text-[#3F5147]">{bank.prepayFee}</dd></div>
                              </dl>
                            </div>
                          </div>

                          <div className="border-t border-[#FDE047] bg-[#FEF9C3] p-4 font-sans text-xs space-y-1.5">
                            <span className="block font-bold text-[#854D0E]">實務提醒</span>
                            {bank.others.map((other, oIdx) => (
                              <div key={oIdx} className="flex items-start gap-1.5 text-[#854D0E] leading-relaxed text-justify">
                    ...
                    <div className="grid grid-cols-1 items-start gap-5 font-sans md:grid-cols-2 xl:grid-cols-3">
                      {japaneseBanks.map((bank, idx) => (
                        <article key={idx} className="w-full border border-[#DDE3DF] bg-[#F5F8F6] transition-all duration-300 hover:border-[#00a174] hover:shadow-colored-soft overflow-hidden">
                          ...
                          <div className="border-t border-[#C9D2CD] p-5 pt-4 md:min-h-[420px] xl:min-h-[470px]">
                            ...
                            <div className="mt-4 pt-3 border-t border-dashed border-[#C9D2CD] text-[11px] text-[#66736C] leading-relaxed text-justify space-y-2">
