...
const AnswerBlock = ({ text }: { text: string; key?: string | number }) => {
  ...
  if (standaloneHeading) return <div className="mt-4 mb-2 font-bold text-[#1A2A22]">{renderFormattedText(standaloneHeading[1])}</div>;

  const lines = trimmed.split("\n").map(line => line.trim()).filter(Boolean);

  return (
    <div className="mt-3 space-y-2 text-[#3F5147] leading-relaxed">
      {lines.map((line, index) => {
        const heading = line.match(/^【(.+)】$/);
        if (heading) {
          return <div key={index} className="pt-2 font-bold text-[#1A2A22]">{renderFormattedText(heading[1])}</div>;
        }
        ...
        if (orderedItem) {
          return (
            <div key={index} className="grid grid-cols-[1.5rem_1fr] items-start gap-1">
              <span className="text-[#3F5147]">{orderedItem[1]}.</span>
        ...
        if (bulletItem) {
          return (
            <div key={index} className="grid grid-cols-[1rem_1fr] items-start gap-1">
              <span className="text-[#3F5147]">•</span>
      ...
      <details className="group">
        <summary className="flex cursor-pointer list-none items-start gap-3 p-4 marker:hidden md:p-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            {displaySummary && (
              <p className="mt-1.5 text-xs leading-relaxed text-[#66736C] group-open:hidden">
                {displaySummary}
              </p>
            )}
          </div>
        ...
        <div className="border-t border-dashed border-[#DDE3DF] bg-[#F5F8F6] px-4 pb-5 pt-4 md:px-5">
          ...
          {table && (
            <div className="mt-5 font-sans">
              <h5 className="mb-2 font-bold text-[#1A2A22]">{table.caption}</h5>
                ...
                <table className="min-w-[900px] w-full table-fixed border-collapse text-left text-[11px] leading-relaxed">
                  ...
                  <tbody>
                    {table.rows.map((row, rowIndex) => (
                      <tr key={rowIndex} className={rowIndex % 2 === 0 ? "bg-white" : "bg-[#FAFCFB]"}>
                        <th className="whitespace-normal break-words border-b border-r border-[#DDE3DF] px-3 py-2.5 font-bold text-[#1A2A22]">{row.label}</th>
              ...
              <div className="mt-2 space-y-1 text-[10px] leading-relaxed text-[#66736C]">
                {table.notes?.map(note => <p key={note}>• {note}</p>)}
              </div>
            </div>
          )}
          {sources && sources.length > 0 && (
            <div className="mt-5 border-t border-[#DDE3DF] pt-3 font-sans">
