...
import { PrefectureMunicipalitiesBarChart } from "./PrefectureMunicipalitiesBarChart";
import type { ListingLocationContext } from "../lib/listingLocation";
import { ShieldCheck, ShieldAlert, Info, FileText, Trophy, CheckCircle2, TrendingDown, TrendingUp, MapPin } from "lucide-react";
...
export function PrefectureSafetyCard({ prefecture, location }: PrefectureSafetyCardProps) {
  ...
  const violentGroup = local?.breakdown?.groups.find((g) => g.code === "B" || g.label.includes("粗暴"));
  const annualStreetCrime = violentGroup && local
    ? { count: violentGroup.count, area: local.municipality, period: `${local.year} 年` }
    : undefined;
  ...
  return (
    <div className="space-y-3 font-sans [font-family:var(--font-sans)]">
      ...
      <div className="border border-[#DDE3DF] bg-white p-4 sm:p-5 space-y-4">
        ...
        {local && local.items && local.items.length > 0 ? (
          /* 市區町村直條圖雙卡（採用東京同款直條比較與左卡版式，分隔線與右卡完全對齊） */
          <div className="tokyo-safety-pair grid grid-cols-1 items-stretch gap-3 sm:grid-cols-2">
            ...
            >
              <div className="prefecture-safety-top">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5">
                    <ShieldAlert className="h-4 w-4 shrink-0" style={{ color: config.accent }} />
                  ...
                  >
                    {config.label.replace("全國", prefecture.prefecture)}
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-4 pt-3 pb-2">
                  <div>
                    <div className="text-[11px] font-medium text-[#66736C]">市區町村統計評級</div>
                    <div
                      className="mt-1 flex h-9 items-baseline text-4xl font-black leading-none tracking-tight tabular-nums"
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex h-9 items-baseline text-4xl font-black leading-none tracking-tight tabular-nums text-[#1A2A22]">
                      {effectiveRate}
                      <span className="ml-1 text-base font-bold text-[#1A2A22]">件</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-1 w-full">
                  {([
                    ...
                    { lvl: 4, label: "較低" },
                    { lvl: 5, label: "明顯較低" },
                  ] as const).map(({ lvl, label }, index) => {
                    const gradeLevel = (({ "A+": 5, A: 4, "B+": 4, B: 3, C: 2, D: 1 } as Record<string, number>)[effectiveGrade] ?? 3);
                    const isCurrent = index + 1 === gradeLevel;
                    const isActive = index < gradeLevel;
                    return (
                      <div className="min-w-0 flex-1 flex flex-col items-center gap-1" key={label}>
                        <div
                          className="h-2 w-full"
                          style={{ backgroundColor: isActive ? config.accent : `${config.border}90` }}
                        />
                        <span
                          className={`text-[9px] leading-tight text-center ${isCurrent ? "font-bold" : "font-normal"}`}
                ...
                <div
                  className="prefecture-safety-description mt-3 flex flex-wrap items-center gap-1 text-[11px] leading-relaxed font-bold"
                  style={{ color: config.text }}
                >
              ...
              {/* 直條圖區域：移除中間 Trophy 欄位，分隔線與右卡完全對齊 */}
              <div className="prefecture-safety-bottom min-w-0 w-full max-w-full overflow-hidden space-y-2.5 border-t border-[#DDE3DF] pt-3">
                <PrefectureMunicipalitiesBarChart
                  prefecture={prefecture.prefecture}
                  area={local.municipality}
                  count={local.total}
                  ratePerThousand={local.crimeRatePerThousand}
                  rank={local.rank}
                  totalAreas={local.totalAreas}
                  prefectureAverageRate={local.prefectureAverageRate}
                  items={local.items}
                  gradeStyle={config}
                  year={local.year}
                />
            ...
            <NeighborhoodActivityCard
              ...
              annualStreetCrime={annualStreetCrime}
            />
          </div>
        ) : (
          /* 縣級備援雙卡（無市區町村資料時） */
            ...
            >
              ...
              <div className="prefecture-safety-bottom space-y-3">
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] leading-relaxed text-[#3F5147]">{prefecture.summary}</p>
                </div>
                {/* 與全國平均的視覺對照 */}
