...
export function PrefectureCrimeBreakdownCard({ data, prefecture }: { data: PrefectureCrimeBreakdown; prefecture: string }) {
  const scopeLabel = data.scopeLabel ?? prefecture;
  ...
  return <div className="space-y-3 border border-[#DDE3DF] bg-[#FAFCFB] p-3.5">
    ...
    <div className="divide-y divide-[#DDE3DF]">
      {data.groups.map(group => group.items.length > 0 ? <details key={group.code} className="group py-2">
      ...
      </details> : <div key={group.code} className="flex flex-wrap items-center gap-2 py-2 text-[11px] text-[#1A2A22]">
        <span className="h-2 w-2 shrink-0" style={{ backgroundColor: COLORS[group.code] }} />
        <span className="min-w-0 flex-1 font-bold">{group.label}</span>
        <span className="font-bold tabular-nums">{group.count.toLocaleString()} 件</span>
        <span className="w-14 text-right tabular-nums text-[#66736C]">{group.percent.toFixed(2)}%</span>
    ...
    <p className="flex items-start gap-1.5 text-[10px] leading-relaxed text-[#66736C]"><ListOrdered className="mt-0.5 h-3 w-3 shrink-0" />有箭頭的分類可查看官方細項。六類件數合計等於總數；百分比四捨五入可能有尾差。</p>
    <p className="text-[10px] leading-relaxed text-[#66736C]">來源：<a href={data.sourceUrl} target="_blank" rel="noreferrer" className="underline">{data.sourceLabel ?? "警察廳「令和5年的犯罪」第3、5表"}</a>。範圍為{scopeLabel}{isMunicipal ? "全區" : "全縣"}，非物件所在町丁目；「風俗犯罪」是官方刑法分類，不代表風俗店數量。</p>
  </div>;
}
