...
const buyHouseQAItems: BuyHouseQAItem[] = [
  ...
  },
  {
    ...
    answer: "先把總取得成本拆清楚：買賣價金、由誰負擔的貸款本金，以及直接構成取得對價的支出，再依可證明的實際負擔比例討論登記持分。父母替子女支付的頭期款、尾款或日後代繳子女應負擔的貸款，都可能形成新的贈與判斷。\n\n不要先登記各半，再事後補借據；也不要只看誰是貸款名義人，而忽略實際由誰還款。簽約前應讓司法書士確認登記持分，並讓日本稅理士檢查出資、貸款與持分是否一致。跨境資金還要保存匯款水單、親屬關係、資金來源、借貸或贈與文件。",
    sources: [
