...
import { 
  ...
  Instagram, Facebook, AtSign, Building, Landmark, Percent, Map, HelpCircle as QIcon
} from "lucide-react";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-serif select-text">
      ...
      <div className="h-1.5 bg-[#b8241d] w-full" id="top-accent-line" />

      {/* Elegant Linear Header */}
      <header className="border-b border-[#1a1a18] bg-white" id="app-header">
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-start gap-4">
            ...
            <div className="border-2 border-[#b8241d] text-[#b8241d] px-2 py-3 font-bold text-center leading-tight tracking-widest text-lg bg-white select-none shrink-0" id="hanko-logo">
              世嘉
              <br />
              不動產
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs bg-[#1a1a18] text-[#faf8f5] px-1.5 py-0.5 font-sans uppercase tracking-widest select-none">Linus 住好日</span>
                <span className="text-xs border border-[#b8241d] text-[#b8241d] px-1.5 py-0.5 select-none font-sans">東京2025新版</span>
              </div>
              <h1 className="text-3xl font-bold tracking-tight text-[#1a1a18] mt-1">日本租房知識大補帖</h1>
              <p className="text-sm text-zinc-600 mt-1 font-sans">
          ...
          <div className="flex flex-col items-end gap-1.5 text-right font-sans border-l-0 md:border-l border-zinc-200 md:pl-6 py-1 w-full md:w-auto">
            <div className="text-xs text-zinc-500 uppercase tracking-wider">張係長 Linus 線上諮詢</div>
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="bg-[#b8241d] hover:bg-[#961a14] text-white px-3 py-1 text-xs font-medium cursor-pointer transition-colors"
        ...
        <nav className="border-t border-[#1a1a18] bg-[#fdfcfb]" id="primary-nav">
          <div className="max-w-6xl mx-auto px-4">
            <div className="grid grid-cols-5 text-center">
              <button 
                onClick={() => handleTabChange("cards")}
                className={`py-4 text-xs md:text-sm font-medium tracking-wide border-r border-[#1a1a18] first:border-l hover:bg-[#faf8f5] transition-colors relative cursor-pointer ${
                  activeTab === "cards" ? "bg-white font-bold text-[#b8241d]" : "text-zinc-700"
                }`}
                id="nav-tab-cards"
              >
                ...
                {activeTab === "cards" && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#b8241d]" />}
              </button>

              <button 
                onClick={() => handleTabChange("buyHouse")}
                className={`py-4 text-xs md:text-sm font-medium tracking-wide border-r border-[#1a1a18] hover:bg-[#faf8f5] transition-colors relative cursor-pointer ${
                  activeTab === "buyHouse" ? "bg-white font-bold text-[#b8241d]" : "text-zinc-700"
                }`}
                id="nav-tab-buyhouse"
              >
                <div className="flex flex-col md:flex-row items-center justify-center gap-1.5">
                ...
                {activeTab === "buyHouse" && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#b8241d]" />}
              </button>

              <button 
                onClick={() => handleTabChange("calculator")}
                className={`py-4 text-xs md:text-sm font-medium tracking-wide border-r border-[#1a1a18] hover:bg-[#faf8f5] transition-colors relative cursor-pointer ${
                  activeTab === "calculator" ? "bg-white font-bold text-[#b8241d]" : "text-zinc-700"
                }`}
                id="nav-tab-calc"
              >
                <div className="flex flex-col md:flex-row items-center justify-center gap-1.5">
                ...
                {activeTab === "calculator" && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#b8241d]" />}
              </button>

              <button 
                onClick={() => handleTabChange("chat")}
                className={`py-4 text-xs md:text-sm font-medium tracking-wide border-r border-[#1a1a18] hover:bg-[#faf8f5] transition-colors relative cursor-pointer ${
                  activeTab === "chat" ? "bg-white font-bold text-[#b8241d]" : "text-zinc-700"
                }`}
                id="nav-tab-chat"
              >
                <div className="flex flex-col md:flex-row items-center justify-center gap-1.5">
                  <Sparkles className="w-4 h-4 shrink-0 text-[#b8241d]" />
                  <span>AI 智能問答</span>
                </div>
                {activeTab === "chat" && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#b8241d]" />}
              </button>

              <button 
                onClick={() => handleTabChange("contact")}
                className={`py-4 text-xs md:text-sm font-medium tracking-wide border-r border-[#1a1a18] hover:bg-[#faf8f5] transition-colors relative cursor-pointer ${
                  activeTab === "contact" ? "bg-white font-bold text-[#b8241d]" : "text-zinc-700"
                }`}
                id="nav-tab-contact"
              >
                <div className="flex flex-col md:flex-row items-center justify-center gap-1.5">
                ...
                {activeTab === "contact" && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#b8241d]" />}
              </button>
            </div>
          </div>
        </nav>
      ...
      <main className="flex-grow max-w-6xl w-full mx-auto px-4 py-8" id="main-content">
        <AnimatePresence mode="wait">
          ...
          {activeTab === "cards" && (
            ...
            >
              {/* Preface Section */}
              <div className="border border-[#1a1a18] bg-white p-6 md:p-8 relative" id="cards-preface">
                {/* Traditional Japanese Ribbon Flag decoration */}
                <div className="absolute top-0 right-8 bg-[#b8241d] text-white px-3 py-1 text-xs select-none uppercase tracking-widest font-sans">
                  前言 ❀
                </div>
                <h3 className="text-xl font-bold border-b border-[#1a1a18] pb-3 mb-4 flex items-center gap-2">
                  <span>致所有來日本打拼的人</span>
                  <span className="text-[#b8241d] text-sm font-normal">By Linus 營業係長</span>
                </h3>
                <p className="text-zinc-800 leading-relaxed text-justify first-letter:text-2xl first-letter:font-bold first-letter:text-[#b8241d] first-letter:mr-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t border-dashed border-zinc-300 font-sans">
                  <div className="bg-[#faf8f5] p-4 border border-zinc-200">
                    <h4 className="font-bold text-[#b8241d] flex items-center gap-2 text-sm">
                    ...
                    <button 
                      onClick={() => handleTabChange("calculator")}
                      className="mt-3 text-xs font-bold text-[#1a1a18] hover:text-[#b8241d] flex items-center gap-1 cursor-pointer"
                    >
              ...
              <div className="border border-[#1a1a18] bg-white p-4 flex flex-col md:flex-row gap-4 justify-between items-center" id="kb-filter-bar">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-medium cursor-pointer border transition-colors ${
                        kbCategory === cat.id 
                          ...
                          : "bg-white text-zinc-700 border-zinc-300 hover:border-[#1a1a18]"
                      }`}
                    >
                      {cat.label}
                    </button>
                ...
                <div className="relative w-full md:w-72 font-sans">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-1.5 text-sm bg-white border border-[#1a1a18] focus:outline-none focus:ring-1 focus:ring-[#b8241d]"
                  />
                  {searchQuery && (
                    <button 
              ...
              {hasNoResults && (
                <div className="border border-dashed border-zinc-300 bg-white py-12 text-center space-y-4">
                  <HelpCircle className="w-12 h-12 text-[#b8241d] mx-auto opacity-75" />
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-[#1a1a18] text-white text-xs tracking-wider uppercase font-sans font-bold hover:bg-[#b8241d] cursor-pointer"
                  >
                    開啟 AI 對話諮詢
                  </button>
                </div>
              ...
              {filtered.fees.length > 0 && (
                <section className="space-y-4">
                  <h3 className="text-lg font-bold border-l-4 border-[#b8241d] pl-3 flex items-center justify-between">
                    <span>【初期費用名詞介紹】</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filtered.fees.map((fee, idx) => (
                      <div 
                        key={idx} 
                        className="border border-[#1a1a18] bg-white p-5 flex flex-col justify-between hover:shadow-[4px_4px_0px_0px_rgba(26,26,24,1)] transition-all cursor-pointer relative"
                        onClick={() => setSelectedFee(fee)}
                      >
                        <div>
                          <div className="flex justify-between items-start gap-2 mb-2">
                            <h4 className="font-bold text-base text-[#1a1a18]">{fee.name}</h4>
                            {fee.jpName && (
                              <span className="text-xs bg-[#faf8f5] px-1.5 py-0.5 border border-zinc-200 text-zinc-600 font-sans font-medium">{fee.jpName}</span>
                            )}
                          </div>
                        ...
                        {fee.warning && (
                          <div className="mt-3 pt-2.5 border-t border-dashed border-zinc-200 text-xs text-[#b8241d] line-clamp-1 font-sans">
                            {fee.warning}
                          </div>
                        )}
                        <div className="mt-4 flex items-center justify-between text-xs text-zinc-400 font-sans">
                          <span>租屋核心術語</span>
                          <span className="text-zinc-600 flex items-center gap-0.5 hover:text-[#b8241d]">點擊深入 ➔</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <section className="space-y-4 pt-4">
                  ...
                  <div className="space-y-4">
                    {filtered.terms.map((term, idx) => (
                      <div key={idx} className="border border-[#1a1a18] bg-white p-6">
                        <div className="flex justify-between items-center mb-3">
                          <h4 className="text-base font-bold text-[#b8241d] flex items-center gap-2">
                            <span>{term.name}</span>
                            {term.jpName && (
                        ...
                        {term.details && term.details.length > 0 && (
                          <div className="bg-[#faf8f5] p-4 border border-zinc-200 space-y-2.5">
                            {term.details.map((detail, dIdx) => (
                              <div key={dIdx} className="text-xs text-zinc-800 leading-relaxed flex items-start gap-2 font-sans">
                                <span className="text-[#b8241d] font-bold shrink-0">✦</span>
              ...
              {filtered.steps.length > 0 && (
                <section className="space-y-4 pt-4">
                  <h3 className="text-lg font-bold border-l-4 border-[#b8241d] pl-3">
                    <span>【日本租屋正式申請與引渡流程 SOP】</span>
                  </h3>
                  
                  {/* General / Overseas SOP highlight banner */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans text-xs">
                    <div className="bg-white p-5 border border-[#1a1a18] relative">
                      <div className="absolute top-0 right-0 bg-[#1a1a18] text-white px-2 py-0.5 font-bold">海外審查</div>
                      <h4 className="font-bold text-sm text-[#b8241d] mb-2 flex items-center gap-1.5">
                        <span>✈ 飛日前提前申請流程</span>
                      </h4>
                      <p className="text-zinc-600 leading-relaxed text-justify mb-2">
                        適合已取得《在留資格認定證明書》(COE) 或打工度假貼紙，人尚未入境日本的人。能省去入境後的租房等待期，好處是落地即入住！
                      </p>
                      <div className="bg-[#faf8f5] p-3 border border-zinc-200">
                        <span className="font-bold text-[#1a1a18]">流程順序：</span>
                        取得簽證貼紙/COE ➔ 線上挑房 ➔ 提交資料 ➔ 保證公司電話照會 ➔ 審查核准 ➔ 海外匯款初期費用 ➔ 入境領在留卡 ➔ 領鑰匙 ➔ 登記住址。
                      </div>
                    </div>

                    <div className="bg-white p-5 border border-[#1a1a18] relative">
                      <div className="absolute top-0 right-0 bg-[#b8241d] text-white px-2 py-0.5 font-bold">境內審查</div>
                      <h4 className="font-bold text-sm text-[#b8241d] mb-2 flex items-center gap-1.5">
                  ...
                  <div className="border border-[#1a1a18] bg-[#faf8f5] p-6 relative">
                    <h4 className="text-sm md:text-base font-bold text-[#1a1a18] border-b border-zinc-300 pb-3 mb-4 flex items-center gap-2">
                      <FileText className="w-4 h-4 text-[#b8241d]" />
                      <span>【審查所需資料與準備文件對照】</span>
                    </h4>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white p-5 border border-zinc-300 space-y-3">
                        <h5 className="font-bold text-[#b8241d] text-xs uppercase tracking-wider border-b border-zinc-100 pb-1 flex items-center gap-1.5">
                          <span>✈ 海外審查需要資料</span>
                        </h5>
                        <ul className="space-y-2 text-xs text-zinc-700 leading-normal font-sans">
                          <li className="flex items-start gap-1.5">
                            <span className="text-[#b8241d] font-bold">•</span>
                  ...
                  <div className="border border-[#1a1a18] bg-white p-6 relative">
                    <div className="absolute top-0 right-6 bg-[#b8241d] text-white px-2.5 py-0.5 text-xs tracking-widest font-sans font-medium uppercase">
                      9個核心步驟
                    </div>
                    <h4 className="text-base font-bold text-[#1a1a18] border-b border-zinc-200 pb-3 mb-6">
                      日本房屋審查、付款與交屋步驟分解：
                    </h4>
                    
                    <div className="relative border-l-2 border-[#1a1a18] ml-3 pl-6 space-y-8 py-2">
                      {filtered.steps.map((step, idx) => (
                        <div key={idx} className="relative group">
                          {/* Circle node indicator */}
                          <div className="absolute -left-[31px] top-1.5 w-4 h-4 bg-white border-2 border-[#b8241d] group-hover:bg-[#b8241d] transition-colors" />
                          
                          <div className="flex flex-col md:flex-row justify-between items-start gap-2 mb-1.5">
                            <h5 className="font-bold text-sm text-[#b8241d]">{step.name}</h5>
                            <span className="text-xs bg-[#faf8f5] border border-zinc-300 text-zinc-600 px-2 py-0.5 font-sans shrink-0">
                              時程估計: {step.duration}
                            </span>
                          </div>
                    ...
                    <div className="bg-[#faf8f5] p-4 border border-zinc-200 text-xs text-zinc-600 leading-relaxed mt-6 font-sans">
                      <span className="font-bold text-[#b8241d]">★ Linus 實務小提醒：</span>
                    ...
                    {filtered.qa.map((qa, idx) => (
                      <div key={idx} className="border border-[#1a1a18] bg-white p-6">
                        <div className="flex items-start gap-3 mb-3">
                          <span className="bg-[#b8241d] text-white font-sans text-xs font-bold w-5 h-5 flex items-center justify-center shrink-0">Q</span>
                          <h4 className="text-sm md:text-base font-bold text-[#1a1a18]">
                            {qa.question}
                          </h4>
                        </div>
          ...
          {activeTab === "buyHouse" && (
            ...
            >
              {/* Preface Section */}
              <div className="border border-[#1a1a18] bg-white p-6 md:p-8 relative" id="buy-house-preface">
                {/* Traditional Japanese Ribbon Flag decoration */}
                <div className="absolute top-0 right-8 bg-[#b8241d] text-white px-3 py-1 text-xs select-none uppercase tracking-widest font-sans">
                  置產 ❀
                </div>
                ...
                <p className="text-zinc-800 leading-relaxed text-justify first-letter:text-2xl first-letter:font-bold first-letter:text-[#b8241d] first-letter:mr-1 font-serif">
                ...
                <p className="text-zinc-800 leading-relaxed text-justify mt-4">
                  為了協助您精準掌握日本房市脈絡，我特別整理了這份包含「圖紙/規費術語」、「現金與貸款買房完整步驟」、「2025最新台系與日系銀行放款標準」，以及「東京都 23 區最詳盡的民泊民宿新法與旅館業法要求」。歡迎直接查閱或透過 AI 問答隨時向我諮詢！❀
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t border-dashed border-zinc-300 font-sans">
                  ...
                  <div className="bg-[#faf8f5] p-4 border border-zinc-200">
                    ...
                    <p className="text-xs text-zinc-600 mt-1">
                      直接聯絡張係長 Linus 團隊，我們將為您在世嘉不動產內部篩選未公開的優質房源。
                    </p>
                    <button 
                      onClick={() => handleTabChange("contact")}
                      className="mt-3 text-xs font-bold text-[#1a1a18] hover:text-[#b8241d] flex items-center gap-1 cursor-pointer"
              ...
              {/* Grid Control & Search Block */}
              <div className="border border-[#1a1a18] bg-white p-4 flex flex-col md:flex-row gap-4 justify-between items-center" id="buy-filter-bar">
                {/* Horizontal Category selectors */}
                <div className="flex flex-wrap gap-2 w-full md:w-auto font-sans">
                  {[
                    { id: "all", label: "全部內容" },
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {buyFiltered.drawing.map((term, idx) => (
                          ...
                          >
                            <div>
                              <div className="flex justify-between items-start gap-2 mb-2">
                                <h4 className="font-bold text-sm md:text-base text-[#1a1a18]">{term.name}</h4>
                                {term.jpName && (
                                  <span className="text-[10px] md:text-xs bg-[#faf8f5] px-1.5 py-0.5 border border-zinc-200 text-zinc-600 font-sans font-medium">{term.jpName}</span>
                                )}
                              </div>
                            ...
                            <div className="mt-4 flex items-center justify-between text-[10px] text-zinc-400 font-sans border-t border-zinc-100 pt-2">
                              <span>圖紙專有名詞</span>
                              <span className="text-[#b8241d] hover:underline flex items-center gap-0.5">點擊詳情 ❀</span>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {(buyCategory === "all" || buyCategory === "steps") && (
                <div className="space-y-8">
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#1a1a18] pb-4 mb-6 gap-4">
                      <div>
                        <h3 className="text-xl font-bold flex items-center gap-2">
                          <MapPin className="w-5 h-5 text-[#b8241d]" />
                      ...
                      {selectedFlowType === "cash" ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                          {buyHouseCashSteps.map((step, sIdx) => (
                            <div key={sIdx} className="border border-zinc-200 bg-[#faf8f5] p-5 relative hover:border-[#1a1a18] transition-colors">
                              <span className="absolute top-4 right-4 text-3xl font-bold text-[#b8241d]/15 select-none font-sans">
                                {step.step}
                              </span>
                              <h4 className="font-bold text-sm md:text-base text-[#1a1a18] mb-2 flex items-center gap-1.5 font-serif">
                                <span className="text-[#b8241d] font-sans font-bold">{step.step}</span>
                              ...
                              {step.warning && (
                                <p className="mt-2 text-[11px] text-[#b8241d] bg-red-50 p-2 border-l-2 border-[#b8241d] leading-normal font-sans">
                                  {step.warning}
                                </p>
                              )}
                            </div>
                  ...
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8">
                    <h3 className="text-lg font-bold border-b border-[#1a1a18] pb-3 mb-4 flex items-center gap-2">
                      <FileText className="w-5 h-5 text-[#b8241d]" />
                      <span>{signingDocuments.title}</span>
                    </h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-sans">
                      <div className="bg-[#faf8f5] p-5 border border-zinc-200">
                        <h4 className="font-bold text-sm text-[#b8241d] border-b border-zinc-300 pb-2 mb-3 flex items-center gap-1.5">
                          <span className="w-2 h-2 bg-[#b8241d]"></span>
                      ...
                      <div className="bg-[#faf8f5] p-5 border border-zinc-200">
                        <h4 className="font-bold text-sm text-[#1a1a18] border-b border-zinc-300 pb-2 mb-3 flex items-center gap-1.5">
                          <span className="w-2 h-2 bg-[#1a1a18]"></span>
              ...
              {(buyCategory === "all" || buyCategory === "loans") && (
                <div className="space-y-8">
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8 space-y-6">
                    <div>
                      <h3 className="text-xl font-bold flex items-center gap-2">
                        <Landmark className="w-5 h-5 text-[#b8241d]" />
                        <span>2025 最新在日台系銀行放款條件比較（非日本居民專用）</span>
                      </h3>
                    ...
                    <div className="space-y-6">
                      {taiwaneseBanks.map((bank, bIdx) => (
                        <div key={bIdx} className="border border-[#1a1a18] bg-white hover:shadow-[4px_4px_0px_0px_rgba(26,26,24,1)] transition-all overflow-hidden">
                          {/* Card Header */}
                          <div className="bg-[#1a1a18] text-[#faf8f5] px-4 py-3 flex justify-between items-center flex-wrap gap-2">
                            <h4 className="font-bold text-sm md:text-base font-serif">{bank.name}</h4>
                            <span className="text-[10px] bg-[#b8241d] text-white px-2 py-0.5 uppercase tracking-wide font-sans">
                          ...
                          <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 font-sans text-xs text-zinc-700">
                            <div className="space-y-3">
                              ...
                              <p><strong>◉ 財力收入與淨資產門檻：</strong> <span className="text-[#b8241d] font-semibold">{bank.incomeAsset}</span></p>
                          ...
                          {/* Bank Pro/Con Notes */}
                          <div className="bg-[#faf8f5] p-4 border-t border-zinc-200 font-sans text-xs space-y-1.5">
                            <span className="font-bold text-[#b8241d] block">★ Linus 實務分析與點評：</span>
                  ...
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8 space-y-6">
                    <div>
                      <h3 className="text-xl font-bold flex items-center gap-2">
                        <Percent className="w-5 h-5 text-[#b8241d]" />
                        <span>2025 最新日系銀行放款條件比較（非永住長期工作簽證者）</span>
                      </h3>
                      <p className="text-xs text-zinc-500 font-sans mt-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans">
                      {japaneseBanks.map((bank, idx) => (
                        <div key={idx} className="border border-zinc-200 bg-[#faf8f5] p-5 flex flex-col justify-between hover:border-[#1a1a18] transition-colors">
                          <div className="space-y-4">
                            <div className="border-b border-zinc-300 pb-2">
                              <h4 className="font-bold text-sm text-[#b8241d] leading-normal">{bank.name}</h4>
                              <div className="text-lg font-extrabold text-[#1a1a18] mt-1">{bank.rate}</div>
                            </div>
              ...
              {(buyCategory === "all" || buyCategory === "minpaku") && (
                <div className="space-y-8">
                  ...
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8 space-y-6">
                    <div>
                      <h3 className="text-xl font-bold flex items-center gap-2">
                        <Map className="w-5 h-5 text-[#b8241d]" />
                        <span>2025 最新東京都 23 區民泊新法營運規範一覽（非旅館業法經營）</span>
                      </h3>
                      <p className="text-xs text-zinc-500 font-sans mt-1">
                    ...
                    <div className="overflow-x-auto border border-zinc-300 font-sans">
                      <table className="w-full text-xs text-left text-zinc-700 border-collapse">
                        <thead className="bg-[#1a1a18] text-[#faf8f5] uppercase tracking-wider text-[10px]">
                          <tr>
                        ...
                        <tbody className="divide-y divide-zinc-200">
                          {minpakuRules.map((item, idx) => (
                            <tr key={idx} className="hover:bg-zinc-50 transition-colors">
                              <td className="py-3 px-4 font-bold text-zinc-900 border-r border-zinc-200 bg-[#faf8f5]">{item.district}</td>
                              <td className="py-3 px-4 font-semibold text-[#b8241d] border-r border-zinc-200">{item.daysLimit}</td>
                    ...
                    <div className="bg-red-50 p-4 border-l-4 border-[#b8241d] text-xs text-[#b8241d] leading-relaxed font-sans">
                  ...
                  <section className="border border-[#1a1a18] bg-white p-6 md:p-8 space-y-6">
                    <div className="border-b border-zinc-200 pb-3">
                      <h3 className="text-xl font-bold flex items-center gap-2">
                        <Building className="w-5 h-5 text-[#b8241d]" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 font-sans">
                      ...
                      <div className="md:col-span-7 space-y-4">
                        <span className="font-bold text-sm text-[#1a1a18] block">◎ 旅館業許可申請 5 個核心階段：</span>
                        <div className="space-y-3 text-xs">
                          {ryokanRules.steps.map((step, sIdx) => (
                            <div key={sIdx} className="bg-[#faf8f5] p-3 border border-zinc-200 flex gap-3">
                              <span className="font-bold text-[#b8241d] shrink-0 text-sm font-sans">0{sIdx+1}</span>
                      ...
                      <div className="md:col-span-5 space-y-4">
                        <span className="font-bold text-sm text-[#1a1a18] block">◎ 簡易宿所硬體與消防規範：</span>
                        <div className="bg-[#faf8f5] p-5 border border-zinc-200 space-y-4 text-xs">
                    ...
                    <div className="space-y-4">
                      {buyFiltered.qa.map((qa, idx) => (
                        <div key={idx} className="border border-[#1a1a18] bg-white p-6">
                          <div className="flex items-start gap-3 mb-3">
                            <span className="bg-[#b8241d] text-white font-sans text-xs font-bold w-5 h-5 flex items-center justify-center shrink-0">Q</span>
                            <h4 className="text-sm md:text-base font-bold text-[#1a1a18] font-serif">
                              {qa.question}
                            </h4>
                          </div>
                          <div className="flex items-start gap-3 pl-8">
          ...
          {activeTab === "calculator" && (
            ...
            >
              ...
              <div className="flex border border-[#1a1a18] bg-[#faf8f5] p-1 gap-1" id="calc-mode-switcher font-sans">
                <button
              ...
              <div className="border border-[#1a1a18] bg-white p-6" id="calc-intro">
                <h3 className="text-lg font-bold border-b border-[#1a1a18] pb-3 mb-4 text-[#1a1a18] flex items-center gap-2 font-sans">
                  <Calculator className="w-5 h-5 text-[#b8241d]" />
                  {calcMode === "rent" ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start" id="calc-engine-container">
                ...
                <div className="xl:col-span-7 space-y-6">
                  ...
                  <div className="border border-[#1a1a18] bg-white p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5 font-sans">
                        ...
                        <select 
                          ...
                          className="w-full bg-white border border-[#1a1a18] py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-[#b8241d] rounded-none cursor-pointer font-sans"
                        >
                      ...
                      <div className="space-y-1.5 font-sans">
                        ...
                        <div className="grid grid-cols-4 border border-[#1a1a18]">
                          {[
                            { id: "r1", label: "1R" },
                            { id: "k1", label: "1K" },
                            { id: "ldk1", label: "1LDK" },
                      ...
                      return (
                        <div className="space-y-1.5 font-sans pt-3 border-t border-dashed border-zinc-200">
                          <label className="text-xs font-bold text-zinc-700 flex items-center gap-1.5">
                            <MapPin className="w-3.5 h-3.5 text-[#b8241d]" />
                    ...
                    <div className="text-xs text-zinc-500 flex items-center gap-1.5 bg-[#faf8f5] p-3 border border-zinc-200 leading-normal font-sans">
                      <Info className="w-4 h-4 text-[#b8241d] shrink-0" />
                      {calcMode === "rent" ? (
                        <span>
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    className="mt-0.5 accent-[#b8241d]"
                                  />
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex-grow">
                                    ...
                                    <div className="text-[10px] text-[#b8241d] font-bold mt-1 font-mono">
                ...
                <div className="xl:col-span-5 xl:sticky xl:top-8 space-y-6">
                  ...
                  <div className="border border-[#1a1a18] bg-white p-6 relative">
                    ...
                    {calcMode === "rent" ? (
                      <>
                        ...
                        {/* The Big Number */}
                        <div className="border-b border-[#1a1a18] pb-4 mb-4">
                          <div className="flex items-baseline gap-1 font-sans">
                            <span className="text-3xl md:text-4xl font-extrabold text-[#b8241d] tracking-tight font-mono">
                              {getCalculatedRent().toLocaleString()}
                            </span>
                            <span className="text-base font-bold text-[#1a1a18]">日圓 / 月</span>
                          </div>
                        ...
                        <div className="space-y-3.5 text-xs font-sans">
                          ...
                          {calcStation !== "none" && (
                            <div className="flex justify-between items-baseline font-sans border-t border-dashed border-zinc-100 pt-3">
                              ...
                              {(() => {
                                ...
                                return (
                                  <span className={`font-bold font-mono ${adjustedPrice >= 0 ? "text-[#b8241d]" : "text-green-700"}`}>
                                    {adjustedPrice >= 0 ? "+" : ""}
                          ...
                          <div className="pt-4 border-t border-dashed border-zinc-300">
                            <span className="text-[#b8241d] font-bold block mb-1">🗂 估算初期費用區間：</span>
                            <div className="bg-[#faf8f5] p-3 border border-zinc-200 space-y-1.5">
                              ...
                              <div className="flex justify-between font-bold text-[#b8241d] font-sans text-[11px] md:text-xs">
                                <span>正常 5 倍平均 (0禮金或0押金之一):</span>
                                <span className="font-mono text-xs text-[#b8241d]">{(getCalculatedRent() * 5).toLocaleString()} 円</span>
                              </div>
                              <div className="flex justify-between font-bold text-zinc-800 font-sans text-[11px] md:text-xs">
                        ...
                        <div className="mt-6 pt-2 font-sans">
                          <button 
                            ...
                            className="w-full bg-[#1a1a18] text-white py-3 px-4 font-bold tracking-wider hover:bg-[#b8241d] cursor-pointer text-xs uppercase transition-colors"
                            id="calc-send-to-ai"
                          >
                            帶入此預算諮詢 AI 房仲 ➔
                          </button>
                      ...
                      <>
                        ...
                        <div className="border-b border-[#1a1a18] pb-4 mb-4">
                          <div className="flex items-baseline gap-1 font-sans">
                            ...
                            <span className="text-base font-bold text-[#1a1a18]">萬日圓</span>
                          </div>
                          <div className="text-xs text-zinc-500 mt-1.5 font-sans leading-relaxed">
                        ...
                        <div className="space-y-4 text-xs font-sans">
                          ...
                          <div className="pt-3 border-t border-dashed border-zinc-200">
                            <span className="text-[#b8241d] font-bold block mb-1">🗂 估算初期過戶費用 (諸費用 - 一次性)：</span>
                            <div className="bg-[#faf8f5] p-3 border border-zinc-200 space-y-1.5">
                          ...
                          <div className="pt-3 border-t border-dashed border-zinc-200">
                            <span className="text-[#b8241d] font-bold block mb-1">🏦 銀行貸款與月還款額試算：</span>
                            <div className="bg-zinc-50 p-3 border border-zinc-200 space-y-1.5">
                              ...
                              <div className="flex justify-between font-bold text-[#b8241d] text-[11px] md:text-xs border-t border-dashed border-zinc-200 pt-1.5 mt-1">
                                <span>估計每月還款 (利率2.2% 20年):</span>
                                <span className="font-mono text-[#b8241d]">{getMonthlyPayment(getCalculatedBuyPrice()).toLocaleString()} 円 / 月</span>
                              </div>
                  ...
                  <div className="bg-white border border-[#1a1a18] p-5 space-y-3 font-sans">
                    <h5 className="font-bold text-sm text-[#1a1a18] flex items-center gap-1.5">
                      <Smile className="w-4.5 h-4.5 text-[#b8241d]" />
                      {calcMode === "rent" ? (
                    ...
                    <div className="text-xs text-zinc-600 space-y-2.5 font-sans leading-relaxed">
                      {calcMode === "rent" ? (
                        <>
                          ...
                          <p className="border-t border-zinc-200 pt-2 text-[#b8241d] font-bold">
                            精算預算 = 133,000 日圓 / 月
                          </p>
          ...
          {activeTab === "chat" && (
            <motion.div
              ...
              id="pane-chat"
            >
              <div className="border border-[#1a1a18] bg-white p-6 relative" id="chat-header-card">
                <div className="absolute top-0 right-8 bg-[#b8241d] text-white px-3 py-1 text-xs tracking-widest font-sans">
                  智能 AI
                </div>
                <h3 className="text-lg font-bold border-b border-[#1a1a18] pb-3 mb-3 text-[#1a1a18] flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-[#b8241d]" />
              ...
              <div className="border border-[#1a1a18] bg-white h-[600px] flex flex-col justify-between overflow-hidden" id="chat-box-interface">
                {/* Message list area */}
                <div className="flex-grow overflow-y-auto p-4 md:p-6 bg-[#fafcfb] space-y-6" id="chat-messages-scroll-area">
                  {chatMessages.map((msg, index) => (
                    ...
                    >
                      ...
                      <div className={`w-8 h-8 flex items-center justify-center shrink-0 font-sans text-xs border ${
                        msg.role === "user" 
                          ? "bg-zinc-800 text-[#faf8f5] border-zinc-800" 
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className={`p-4 text-xs md:text-sm leading-relaxed text-justify whitespace-pre-wrap border ${
                          msg.role === "user" 
                            ...
                            : "bg-[#fffdfa] border-[#1a1a18] text-zinc-900 shadow-[2px_2px_0px_0px_rgba(26,26,24,1)]"
                        }`}>
                  ...
                  {chatLoading && (
                    <div className="flex gap-3.5 max-w-[80%] mr-auto">
                      <div className="w-8 h-8 flex items-center justify-center bg-[#b8241d] text-white border border-[#b8241d] font-sans text-xs shrink-0 animate-pulse">
                        林
                      </div>
                      <div className="space-y-1">
                        <div className="text-[10px] text-zinc-400 font-sans">Linus 正在調閱日本不動產知識庫...</div>
                        <div className="p-3 bg-[#fffdfa] border border-[#1a1a18] text-xs text-zinc-500 font-sans italic animate-pulse">
                          正在整理租賃規則中，請稍候片刻...
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-[#faf8f5] border-t border-zinc-200 p-3 flex flex-wrap gap-1.5 overflow-x-auto select-none" id="chat-quick-suggestions">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className="bg-white hover:bg-[#fffdfb] border border-zinc-300 hover:border-[#b8241d] text-[11px] text-zinc-700 hover:text-[#b8241d] px-2.5 py-1 transition-colors cursor-pointer font-sans"
                    >
                      {p}
                    </button>
                  ))}
                ...
                <form 
                  ...
                  className="border-t border-[#1a1a18] bg-white p-3 flex gap-2 font-sans"
                  id="chat-send-form"
                >
                  <input
                    type="text"
                    ...
                    className="flex-grow px-4 py-2 text-sm bg-white border border-[#1a1a18] focus:outline-none focus:ring-1 focus:ring-[#b8241d] disabled:bg-zinc-100 disabled:text-zinc-400"
                  />
                  <button
                    type="submit"
                    disabled={chatLoading || !chatInput.trim()}
                    className="bg-[#1a1a18] hover:bg-[#b8241d] text-white px-5 py-2 text-xs font-bold uppercase tracking-wider transition-colors disabled:bg-zinc-300 disabled:text-zinc-500 flex items-center gap-1 cursor-pointer shrink-0"
                    id="chat-submit-btn"
                  >
                    <span>發送</span>
          ...
          {activeTab === "contact" && (
            ...
            >
              {/* Preface section */}
              <div className="border border-[#1a1a18] bg-white p-6" id="contact-intro">
                <h3 className="text-lg font-bold border-b border-[#1a1a18] pb-3 mb-3 text-[#1a1a18] flex items-center gap-2">
                  <Smile className="w-5 h-5 text-[#b8241d]" />
                  <span>世嘉不動產 精英華人房仲團隊為您服務</span>
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start" id="contact-business-layout">
                ...
                <div className="md:col-span-5 space-y-4">
                  ...
                  <div className="border-2 border-[#1a1a18] bg-white p-6 relative shadow-[6px_6px_0px_0px_rgba(26,26,24,1)]" id="meishi-card">
                    ...
                    <div className="space-y-5 mt-2 relative z-10">
                      <div className="space-y-1">
                        ...
                        <h4 className="text-xl font-bold tracking-tight text-[#1a1a18]">{linusContact.name}</h4>
                        <div className="text-xs font-bold text-[#b8241d] font-sans">{linusContact.title}</div>
                      </div>

                      <div className="space-y-3.5 text-xs text-zinc-700 font-sans border-t border-zinc-200 pt-4">
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-[#1a1a18] w-20 shrink-0 tracking-wider">LINE ID</span>
                          <span className="font-mono bg-zinc-100 px-2 py-0.5 border border-zinc-200 font-semibold">{linusContact.lineId}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-[#1a1a18] w-20 shrink-0 tracking-wider">WECHAT</span>
                          <span className="font-mono bg-zinc-100 px-2 py-0.5 border border-zinc-200 font-semibold">{linusContact.wechatId}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-[#1a1a18] w-20 shrink-0 tracking-wider">EMAIL</span>
                          <span className="font-mono text-zinc-600">{linusContact.email}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-[#1a1a18] w-20 shrink-0 tracking-wider">PHONE</span>
                        ...
                        <div className="flex items-center gap-3 pt-3.5 border-t border-dashed border-zinc-300">
                          <span className="font-bold text-[#1a1a18] w-20 shrink-0 tracking-wider">SOCIALS</span>
                          <div className="flex items-center gap-4">
                            <a 
                              href="https://www.facebook.com/r352410/" 
                              target="_blank" 
                              rel="noopener noreferrer" 
                              className="text-zinc-600 hover:text-[#b8241d] hover:scale-110 transition-transform p-1"
                              title="Facebook"
                            >
                  ...
                  {/* Copy Line block */}
                  <div className="border border-[#1a1a18] bg-white p-4 space-y-3 font-sans text-xs">
                    <span className="font-bold text-[#1a1a18] block">直接添加 LINE 諮詢：</span>
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={handleCopyLine}
                        className="bg-[#b8241d] hover:bg-[#961a14] text-white px-4 py-2 font-bold cursor-pointer transition-colors shrink-0"
                  ...
                  {/* Copy WeChat block */}
                  <div className="border border-[#1a1a18] bg-white p-4 space-y-3 font-sans text-xs">
                    <span className="font-bold text-[#1a1a18] block">直接添加 WeChat 諮詢：</span>
                    <div className="flex gap-2">
                      <div className="bg-zinc-100 flex-grow px-3 py-2 border border-zinc-300 font-mono font-bold text-center select-all">
                  ...
                  <div className="border border-[#1a1a18] bg-white p-4 font-sans text-xs space-y-4">
                    {/* Toggle Selector */}
                    <div className="flex border border-[#1a1a18] text-[11px] font-bold">
                      <button
                    ...
                    {contactFormType === "rent" ? (
                      <div className="space-y-2">
                        <span className="font-bold text-[#b8241d] block">📋 諮詢租房時建議先準備好以下資料：</span>
                        <p className="text-zinc-600 leading-normal text-justify">
                          為了讓 Linus 能更快速地幫您向管理公司卡位優質好房，歡迎直接複製並填寫以下諮詢表傳送給 Linus 喔！
                        </p>
                        
                        <div className="bg-[#1a1a18] text-white p-4 text-[11px] leading-relaxed select-all border border-[#1a1a18] font-mono whitespace-pre-line">
                          {`1. 期望入住日期：
...
7. 其他對房子的核心條件（如：獨立洗面台、屋齡限制等）：`}
                      ...
                      <div className="space-y-2">
                        <span className="font-bold text-[#b8241d] block">📋 諮詢買房時建議先準備好以下資料：</span>
                        <p className="text-zinc-600 leading-normal text-justify">
...
7. 其他房子的要求（屋齡／大小／樓層）：`}
                        ...
                        <div className="bg-red-50 p-3 border-l-2 border-[#b8241d] space-y-1.5 text-[11px] leading-relaxed">
                          <span className="font-bold text-[#b8241d] block">貸款注意事項⚠️</span>
                ...
                <div className="md:col-span-7 border border-[#1a1a18] bg-white p-6 space-y-6">
                  <div>
                    <h4 className="font-bold text-base text-[#1a1a18] border-b border-[#1a1a18] pb-2 mb-4">
                      🎌 不動產會社基本資料
                    </h4>
                  ...
                  <div className="border-t border-zinc-200 pt-4 space-y-4">
                    <div>
                      <h4 className="font-bold text-sm text-[#1a1a18] mb-2 font-sans uppercase tracking-wider">
                        📍 公司所在地及交涉站點：
                      </h4>
                    ...
                    <div className="border border-[#1a1a18] bg-[#faf8f5] p-2 relative">
                      <iframe 
                      ...
                      <div className="mt-2 flex justify-between items-center text-xs font-sans">
                        ...
                        <a 
                          ...
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="text-[#b8241d] hover:text-[#1a1a18] font-bold flex items-center gap-1 hover:underline transition-colors py-1 px-2 border border-zinc-200 bg-white"
                    ...
                    <div className="bg-[#faf8f5] p-4 border border-zinc-200 space-y-2">
        ...
        {selectedFee && (
          ...
          >
            <motion.div 
              ...
              className="bg-white border-2 border-[#1a1a18] w-full max-w-lg p-6 relative rounded-none shadow-[8px_8px_0px_0px_rgba(26,26,24,1)]"
              id="term-modal-content"
            >
              <button 
                onClick={() => setSelectedFee(null)}
                className="absolute top-4 right-4 text-zinc-500 hover:text-[#1a1a18] cursor-pointer"
                id="term-modal-close"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b border-zinc-200 pb-2.5">
                  <h4 className="text-xl font-bold text-[#1a1a18]">{selectedFee.name}</h4>
                  {selectedFee.jpName && (
                    <span className="text-xs bg-[#b8241d] text-white px-1.5 py-0.5 font-sans">{selectedFee.jpName}</span>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {selectedFee.warning && (
                    <div className="bg-red-50 border-l-4 border-[#b8241d] p-3 text-xs text-[#b8241d] leading-relaxed font-sans">
                ...
                <div className="pt-4 border-t border-zinc-100 flex justify-end gap-3 font-sans">
                  <button
                    ...
                    className="px-4 py-2 border border-[#1a1a18] text-zinc-800 hover:bg-[#faf8f5] text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors"
                  >
                    向 AI 問答助理諮詢此費用
                  </button>
                  <button
                    onClick={() => setSelectedFee(null)}
                    className="px-4 py-2 bg-[#1a1a18] text-white hover:bg-[#b8241d] text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors"
                  >
                    關閉視窗
                  </button>
                </div>
      ...
      <footer className="border-t border-[#1a1a18] bg-white mt-12 py-12 px-4" id="app-footer">
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
            <div className="space-y-1">
              <strong className="text-sm text-[#1a1a18]">LINUS住好日 ╳ 日本租房知識大補帖</strong>
              <p className="text-xs text-zinc-500 font-sans">
                版權所有 © 2025 LINUS Nice Day Japan All Rights Reserved. 株式会社世嘉 Seika (東京都知事免許第111940号)
              </p>
            </div>
            
            <div className="flex gap-4 text-xs font-sans">
              <a href="https://www.threads.net" target="_blank" rel="noopener noreferrer" className="text-zinc-600 hover:text-[#b8241d] flex items-center gap-0.5">
                <span>Threads 專頁</span> <ExternalLink className="w-3 h-3" />
              </a>
              <span className="text-zinc-300">|</span>
              <a href={`mailto:${linusContact.email}`} className="text-zinc-600 hover:text-[#b8241d]">
                電子信箱
              </a>
            </div>
          </div>

          <div className="bg-[#faf8f5] border border-zinc-200 p-4 text-[10px] text-zinc-500 leading-relaxed text-justify font-sans">
            <p>
