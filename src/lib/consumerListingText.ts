...
export function consumerListingText(value: string): string {
  ...
  const filtered = parts.filter(part => {
      ...
      if (/取引態[様樣]|取引形態|交易態[様樣]|交易形態|引取態[様樣]|定休日|定休[:：]|ネット掲載|ネット転載|網路刊登|刊登限制|広告(?:掲載|転載|可|不可)|廣告(?:刊登|轉載|刊載|可|不可)|客付|元付|仲介会社様|業者様|物確|内見予約|內見預約|内見方法|司法書士.*(?:指定|弊社)|(?:指定|弊社).*司法書士/i.test(text)) return false;
