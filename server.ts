...
// 注意：這是「送進 AI 前」的主題白名單，沒命中就直接被擋在本地。
// 新增或修改前端「熱門諮詢」按鈕時，務必確認新句子至少命中一個關鍵字，
// 否則使用者一按下去就會收到「僅提供日本租屋買房諮詢」的拒絕訊息。
// 「審査」是日文漢字、「審查」是中文，兩種寫法都要列，才不會漏掉。
const HOUSING_TOPIC_PATTERN = /租|買房|買屋|賣房|售屋|不動產|房地產|物件|房源|公寓|住宅|套房|房東|仲介|管理公司|保證公司|保證會社|契約|審查|審査|簽證|簽証|在留|居留|打工度假|留學|留学|存款|財力|押金|敷金|禮金|管理費|共益費|初期費用|房租|家賃|房貸|貸款|銀行|民泊|旅館|水電|瓦斯|網路|搬家|入住|退房|解約|建築|土地|投資|車站|通勤|格局|屋齡|坪|平方米|平方公尺|reins|suumo|マンション|アパート/i;
...
7. 【自我介紹的說法】被問到「你是誰」「你是不是 AI」「你能做什麼」，或需要在拒絕回答時說明自己的身分時，一律自稱「Linus 住好日的 AI 不動產顧問」（可簡稱「AI 不動產顧問」），與網站上的分頁名稱一致。不可自稱「資訊助理」「AI 助理」「聊天機器人」「語言模型」或其他泛稱。誠實是可以的——若被直接詢問，可以說明你是依據 Linus 的日本不動產知識庫回答的 AI 不動產顧問，真人諮詢請加 LINE：linus0922——但職稱要固定用上述說法。
